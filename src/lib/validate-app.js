/* Per-entry schema check for one app object: shape, vocabularies, and the
   fields the pages actually read. scripts/validate-apps.mjs runs this over
   every file in data/apps (CI), and the submission pipeline runs it over
   AI-drafted entries before a PR is opened — one set of rules, two callers. */
import { CATEGORIES, MOAT_TAGS, VERDICTS } from './apps.js';
...
// Returns a list of problem strings; empty means the entry passes.
